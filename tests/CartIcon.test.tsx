...
import { CartItem, SetCartFn } from '../src/types';
...
describe('Cart Icon', () => {
  it('renders a shopping cart button', () => {
    render(<CartIcon cart={[]} setCart={setCart as SetCartFn} />);
  ...
  it('has an accurate number badge if cart has items', () => {
    render(<CartIcon cart={exampleCart} setCart={setCart as SetCartFn} />);
  ...
  it('does not display modal before click', () => {
    render(<CartIcon cart={exampleCart} setCart={setCart as SetCartFn} />);
    expect(
      screen.queryByRole('table', { name: /items in your cart/i }),
    ).toBeNull();
  });
...
describe('Cart Modal', () => {
  ...
  it('shows all items added to cart', async () => {
    ...
    exampleCart.forEach((item) => {
      ...
      expect(
        screen.getByRole('cell', { name: String(item.quantity) }),
      ).toBeInTheDocument();
      expect(
  ...
  it('removes item when "Remove" button is clicked', async () => {
    ...
    const { rerender } = render(
      <CartIcon cart={cart} setCart={setCart as SetCartFn} />,
    );
    ...
    for (let i = 0; i < exampleCart.length; i++) {
      ...
      rerender(<CartIcon cart={cart} setCart={setCart as SetCartFn} />);
